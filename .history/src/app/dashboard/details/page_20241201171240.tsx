...
export default function Details() {
  ...
  const TOKEN_NAMES = ["Moonwell", "USDC", "WELL", "mUSDC"];

  const generateTotalRewardsObject = () => {    
    console.log(details?.totalRewardsObject);
    
    return details?.totalRewardsObject.map((reward: any, index: number) => ({
      ...reward,
      tokenName: TOKEN_NAMES[index] || `Token ${index + 1}`
    }));
  };
  ...
  useEffect(() => {
    ...
    if (dashboardData) {
      ...
      //
      const totalRewardsObject = generateTotalRewardsObject();  
      console.log(totalRewardsObject);
      //
