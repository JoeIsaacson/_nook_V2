...
export default function Dashboard() {
  ...
  const fetchLendingData = useCallback(() => {
    ...
    // First, let's check what chains are available for the protocol
    fetch(
      `https://pro-openapi.debank.com/v1/protocol?id=bsc_helmet`,
      {
        headers: {
          'accept': 'application/json',
          'AccessKey': apiKey || ''
    ...
    .then(res => res.json())
    .then(data => {
      console.log('Protocol Info:', data); // This will show supported chains
    });

    // Then make our main call with explicit chain parameter
    fetch(
      `https://pro-openapi.debank.com/v1/user/protocol?id=${address}&protocol_id=bsc_helmet&chain_ids[]=bsc`,
      {
        headers: {
          'accept': 'application/json',
          'AccessKey': apiKey || ''
    ...
    .then(data => {
        ...
        if (data.portfolio_item_list && data.portfolio_item_list.length > 0) {
          ...
          console.log('No data found for Base chain');
        }
    })
    .catch(err => {
